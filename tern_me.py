...
# 
# ztc 20161117 if it is a Function, 
# it can directly preview some of its parameters now, rather than (fn/1) (fn/2)...
# And align the type to the right will looks more comfortable.
# 
# 
...
def pfile_modified(pfile, view):
  ...
  if now - pfile.last_modified > .5:
    ...
    if is_st2:
      sublime.set_timeout(lambda: maybe_save_pfile(pfile, view, now), 15000)
    else:
      sublime.set_timeout_async(lambda: maybe_save_pfile(pfile, view, now), 15000)
...
def ensure_completions_cached(pfile, view):
  ...
  for rec in data["completions"]:
    ...
    if arg_completion_enabled and rec_type is not None and rec_type.startswith("fn("):
      ...
      # ztc remove other arguments: 3 only 3 dont use 2,1,0 etc.
